...
class MalletPhotoCaptureNode(Node):
    def __init__(self):
        super().__init__('rockpick_photo_capture_node')

        self.save_dir = os.path.join(os.path.expanduser('~'), 'rock_picks')
        ...
        self.target_labels = [
            "a rock pick with a yellow and black handle",
            "a silver metal geological hammer",
            "a shiny metal tool with a yellow grip or black grip",
            "a pointed rock hammer on the ground",
            "a yellow or black handled masonry tool"
        ]
                
        self.background_labels = [
        "natural grey rocks and stones",
        "brown dirt and dry soil",
        "green grass or weeds",
        "the shadow of a rock",
        "yellow debris or trash",
        ...
        self.labeled_img_pub = self.create_publisher(Image, '/station/rockpick_captured_image', 10)
    ...
    def is_metallic(self, crop):
        hsv = cv2.cvtColor(crop, cv2.COLOR_BGR2HSV)
        mask = cv2.inRange(hsv, np.array([0, 0, 100]), np.array([180, 50, 255]))
    ...
    def image_callback(self, msg):
        ...
        for i, mask in enumerate(masks.data):
            ...
            if crop.size == 0 or not self.is_metallic(crop):
                continue
            ...
            if target_score > background_score and target_score > self.clip_confidence_threshold:
                ...
                label_str = f"Rock_pick_hammer ({target_score:.2%})"
                cv2.putText(annotated_image, label_str, (x_min, y_min - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 165, 255), 2)

                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = os.path.join(self.save_dir, f"rock_pick_{timestamp}.jpg")

...
class MalletPhotoCaptureNode(Node):
    def __init__(self):
        super().__init__('bottle_photo_capture_node')

        self.save_dir = os.path.join(os.path.expanduser('~'), 'captured_bottles')
        ...
        self.target_labels = [
            ...
            "a 1 liter water bottle on the ground", # Change color if you changed the URDF color
        ...
        self.background_labels = [
            ...
            "jagged rocks and stones",
            "dry dirt and soil",
            "scattered trash or debris",
            "man-made structures",
            "the shadow of a cylindrical object",
            "green vegetation"
        ]
        ...
        self.clip_confidence_threshold = 0.60
        ...
        self.labeled_img_pub = self.create_publisher(Image, '/station/bottle_captured_image', 10)

        self.get_logger().info("Bottle Photo Capture Node Initialized and Weights Loaded.")
    ...
    def is_bottle_shape(self, crop):
        """Checks if the detected segment matches bottle proportions."""
        h, w = crop.shape[:2]
        if w == 0: return False
        
        aspect_ratio = h / w
        return 1.8 < aspect_ratio < 3.2
    ...
    def image_callback(self, msg):
        ...
        for i, mask in enumerate(masks.data):
            ...
            if not self.is_bottle_shape(crop):
                continue
            ...
            if target_score > background_score and target_score > self.clip_confidence_threshold:
                ...
                filename = os.path.join(self.save_dir, f"bottle_{timestamp}.jpg")

...
# Fix hardcoded paths in DAE mesh files (from other user's system)
DAE_FILE="${WORKSPACE_DIR}/src/rover_description/models/cpr_agriculture/meshes/agriculture_world.dae"
if [ -f "$DAE_FILE" ]; then
    if grep -q "file:///home/naman72060" "$DAE_FILE" 2>/dev/null; then
        sed -i "s|file:///home/naman72060/rover_ws/src/rover_description|file://${WORKSPACE_DIR}/src/rover_description|g" "$DAE_FILE"
        echo "✓ Fixed DAE mesh texture paths"
    fi
fi

...
export async function POST(request: Request) {
    try {
        const { launchKey, action, args } = await request.json()
        ...
        if (action === 'start') {
            ...
            const launchFiles: Record<string, string> = {
                ...
                aruco_mission: 'aruco_mission.launch.py'
            }
...
echo "[$(date)] Starting ${launchFile} with args: ${argsStr}..." | tee ${logFile}
ros2 launch rover_description ${launchFile} ${argsStr} 2>&1 | tee -a ${logFile} &
LAUNCH_PID=$!

...
interface ProcessManager {
  ...
  loading: boolean
  error: string | null
  startProcess: (processKey: string, args?: Record<string, string>) => Promise<void>
...
export function useProcessManager(): ProcessManager {
  ...
  const startProcess = async (processKey: string, args?: Record<string, string>) => {
    try {
      setProcesses(prev => ({
        ...prev,
      ...
      const response = await fetch("/api/launch", {
        ...
        body: JSON.stringify({ action: "start", launchKey: processKey, args })
      })
